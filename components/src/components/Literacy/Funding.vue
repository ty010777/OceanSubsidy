...
<script setup>
    ...
    const editable = computed(() => isProjectEditable("literacy", form.value.Status, 3));
    const errors = ref({});
    ...
    const load = () => {
        rxjs.forkJoin([
            api.literacy("getFunding", { Apply: props.apply ? "true" : "false", ID: props.id })
        ]).subscribe((result) => {
            const data = result[0];
    ...
    const save = (submit, confirm) => {
        ...
        api.literacy("saveFunding", data).subscribe(() => {
            if (submit) {
                emit("next");
            } else {
                load();
