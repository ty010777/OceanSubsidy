...
<script setup>
    ...
    const docs = ref([
        { Type: 1, Title: "申請表", Template: `../../Service/OFS/DownloadTemplateLIT.ashx?type=1&id=${props.id}`, Files: [] },
        { Type: 2, Title: "計畫書", Template: `../../Service/OFS/DownloadTemplateLIT.ashx?type=2&id=${props.id}`, Files: [] },
        { Type: 3, Title: "執行承諾書", Template: `../../Service/OFS/DownloadTemplateLIT.ashx?type=3&id=${props.id}`, Files: [] },
        { Type: 4, Title: "未違反公職人員利益衝突迴避法切結書及事前揭露表", Template: `../../Service/OFS/DownloadTemplateLIT.ashx?type=4&id=${props.id}`, Files: [] },
        { Type: 5, Title: "著作權授權同意書", Template: `../../Service/OFS/DownloadTemplateLIT.ashx?type=5&id=${props.id}`, Files: [] },
        { Type: 6, Title: "其他佐證資料", Comment: "提案計畫倘有結合其他合作單位、團體或相關機關(構)，請提供合作意向書或相關參考文件尤佳，無則免附", Files: [], Multiple: true, Optional: true }
    ]);

    const changeForm = ref();
    const disabled = computed(() => docs.value.some((doc) => !doc.Optional && !doc.Files.filter((file) => !file.Deleted).length));
    const editable = computed(() => isProjectEditable("literacy", project.value.Status, 2));
    const errors = ref({});
    const project = ref({});
