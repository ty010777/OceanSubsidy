...
<script setup>
    ...
    const docs = ref([
        { Type: 21, Title: "領據", Template: "../../Template/LIT/1-領據.docx", Files: [] },
        { Type: 22, Title: "經費收支明細表", Template: "../../Template/LIT/2-經費收支明細表.docx", Files: [] },
        { Type: 23, Title: "經費分攤表", Template: "../../Template/LIT/3-經費分攤表.docx", Files: [] },
        { Type: 24, Title: "補助清單", Template: "../../Template/LIT/4-補助清單.docx", Files: [] }
    ]);
    ...
    const load = () => {
        api.literacy("getPayment", { ID: props.id }).subscribe((result) => {
            project.value = result.Project;

            useProgressStore().init("literacy", project.value);
            ...
            prevPaid.value = result.Payments.filter((item) => item.Stage < 2 && item.Status === "通過").reduce((sum, item) => sum + item.CurrentActualPaidAmount, 0);
            max.value = prevPaid.value;
