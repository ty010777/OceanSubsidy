...
<script setup>
    ...
    const editable = computed(() => isProjectEditable("accessibility", form.value.Status, 2));
    const errors = ref({});
    ...
    const addItem = () => api.accessibility("createItem", { ID: props.id }).subscribe((result) => items.value.push(result));
    ...
    const load = () => {
        rxjs.forkJoin([
            api.accessibility("getWorkSchedule", { Apply: props.apply ? "true" : "false", ID: props.id })
        ]).subscribe((result) => {
            const data = result[0];
    ...
    const save = (submit) => {
        ...
        api.accessibility("saveWorkSchedule", data).subscribe(() => {
