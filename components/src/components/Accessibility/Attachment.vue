...
<script setup>
    ...
    const docs = ref([
        { Type: 1, Title: "申請表", Template: `../../Service/OFS/DownloadTemplateACC.ashx?type=1&id=${props.id}`, Files: [] },
        { Type: 2, Title: "計畫書", Template: `../../Service/OFS/DownloadTemplateACC.ashx?type=2&id=${props.id}`, Files: [] },
        { Type: 3, Title: "執行承諾書", Template: `../../Service/OFS/DownloadTemplateACC.ashx?type=3&id=${props.id}`, Files: [] },
        { Type: 4, Title: "未違反公職人員利益衝突迴避法切結書及事前揭露表", Template: `../../Service/OFS/DownloadTemplateACC.ashx?type=4&id=${props.id}`, Files: [] },
    ...
    const editable = computed(() => isProjectEditable("accessibility", project.value.Status, 2));
    const errors = ref({});
    const project = ref({});
    ...
    const load = () => {
        rxjs.forkJoin([
            api.accessibility("getAttachment", { Apply: props.apply ? "true" : "false", ID: props.id })
        ]).subscribe((result) => {
            const data = result[0];
