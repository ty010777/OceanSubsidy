...
<script setup>
    ...
    const docs = ref([
        { Type: 11, Title: "收支明細表", Template: "../../Template/EDC/1-收支明細表.docx", Files: [] },
        { Type: 12, Title: "受補助清單", Template: "../../Template/EDC/2-受補助清單.docx", Files: [] },
        { Type: 13, Title: "經費分攤表", Template: "../../Template/EDC/3-經費分攤表.docx", Files: [] },
        { Type: 14, Title: "憑證", Template: "../../Template/EDC/4-憑證.docx", Files: [] },
        { Type: 15, Title: "領據", Template: "../../Template/EDC/5-領據.docx", Files: [] }
    ]);
    ...
    const add = (item, file) => {
        if (file.Type === "application/pdf") {
            item.Files.push({
                Type: item.Type,
                Path: file.Path,
                Name: `${project.value.ProjectID}_${item.Title}.pdf`
            });
        }
    };
    ...
    const load = () => {
        api.education("getPayment", { ID: props.id }).subscribe((result) => {
            project.value = result.Project;

            useProgressStore().init("education", project.value);
    ...
    const save = (submit) => {
        ...
        api.education("savePayment", data).subscribe((res) => {
            if (res) {
