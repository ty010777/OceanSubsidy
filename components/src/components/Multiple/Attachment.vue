...
<script setup>
    ...
    const docs = ref([
        ...
        { Type: 3, Title: "執行承諾書", Template: `../../Service/OFS/DownloadTemplateMUL.ashx?type=3&id=${props.id}`, Files: [] },
    ...
    const load = () => {
        rxjs.forkJoin([
            api.multiple("getAttachment", { Apply: props.apply ? "true" : "false", ID: props.id })
        ]).subscribe((result) => {
            const data = result[0];
            ...
            if (project.value.Status >= 42) {
                docs.value.push({ Type: 6, Title: "著作權授權同意書", Template: "../../Template/MUL/6-著作權授權同意書.docx", Files: [] });
            }
