    ...
    <div class="block">
        ...
        <div class="table-responsive mb-0">
            <table class="table align-middle gray-table">
                ...
                <tbody>
                    <tr>
                        <td class="text-end">{{ applyAmount }}</td>
                        <td class="text-end">{{ prevPaid }}</td>
        ...
        <template v-if="isApproved">
            ...
            <p class="text-teal fw-bold pb-3">累積實際撥款: {{ totalPaid.toLocaleString() }}元</p>
        </template>
...
<script setup>
    ...
    const disabled = computed(() => amount.value <= 0 || docs.value.some((doc) => !doc.Files.filter((file) => !file.Deleted).length));
    ...
    const rate2 = computed(() => totalPaid.value ? Math.min(Math.round(amount.value * 100 / totalPaid.value * 100) / 100, 100) : 0);
    const totalPaid = computed(() => prevPaid.value + (isApproved.value ? payment.value.CurrentActualPaidAmount : 0));

    const docs = ref([
        { Type: 21, Title: "領據", Template: "../../Template/MUL/1-領據.docx", Files: [] },
        { Type: 22, Title: "經費收支明細表", Template: "../../Template/MUL/2-經費收支明細表.docx", Files: [] },
        { Type: 23, Title: "經費分攤表", Template: "../../Template/MUL/3-經費分攤表.docx", Files: [] },
        { Type: 24, Title: "合作經費清單", Template: "../../Template/MUL/4-合作經費清單.docx", Files: [] }
    ]);
    ...
    const add = (item, file) => {
        if (file.Type === "application/pdf") {
            item.Files.push({
                Type: item.Type,
                Path: file.Path,
                Name: `${project.value.ProjectID}_第二期_${item.Title}.pdf`
            });
        }
    };
    ...
    const load = () => {
        api.multiple("getPayment", { ID: props.id }).subscribe((result) => {
            ...
            payment.value = result.Payments.find((item) => item.Stage === 2);

            if (payment.value) {
                applyAmount.value = payment.value.CurrentRequestAmount;
            } else {
                applyAmount.value = Math.round(project.value.ApprovedAmount * props.setting.DisbursementRatioPct / 100);
            }

            const approved = result.Payments.filter((item) => item.Stage < 2 && item.Status === "通過");

            prevAmount.value = approved.reduce((sum, item) => sum + item.CurrentRequestAmount, 0);
            prevPaid.value = approved.reduce((sum, item) => sum + item.CurrentActualPaidAmount, 0);

            max.value = project.value.ApprovedAmount;

            usePaymentStore().init(payment.value, prevPaid.value);
        });
    };
    ...
    const save = (submit) => {
        const data = {
            Payment: {
                ...
                CurrentRequestAmount: applyAmount.value,
        ...
        api.multiple("savePayment", data).subscribe(() => {
            if (submit) {
                emit("next");
            } else {
                load();
