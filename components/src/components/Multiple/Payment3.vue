            ...
            <table class="table align-middle gray-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div>
                                <required-label>其他資料（5分鐘精華影片）</required-label>
                            </div>
                        </td>
                        <td colspan="2">
...
<script setup>
    ...
    const rate2 = computed(() => prevPaid.value ? Math.min(Math.round(amount.value * 100 / prevPaid.value * 100) / 100, 100) : 0);

    const docs = ref([
        { Type: 31, Title: "領據", Template: "../../Template/MUL/1-領據.docx", Files: [] },
        { Type: 32, Title: "經費收支明細表", Template: "../../Template/MUL/2-經費收支明細表.docx", Files: [] },
        { Type: 33, Title: "經費分攤表", Template: "../../Template/MUL/3-經費分攤表.docx", Files: [] },
        { Type: 34, Title: "合作經費清單", Template: "../../Template/MUL/4-合作經費清單.docx", Files: [] }
    ]);
    ...
    const add = (item, file) => {
        if (file.Type === "application/pdf") {
            item.Files.push({
                Type: item.Type,
                Path: file.Path,
                Name: `${project.value.ProjectID}_結案核銷_${item.Title}.pdf`
            });
        }
    };
    ...
    const load = () => {
        api.multiple("getPayment", { ID: props.id }).subscribe((result) => {
            ...
            payment.value = result.Payments.find((item) => item.Stage === 3);
            prevPaid.value = result.Payments.filter((item) => item.Stage < 3 && item.Status === "通過").reduce((sum, item) => sum + item.CurrentActualPaidAmount, 0);
