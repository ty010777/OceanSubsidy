<template>
    <div class="block">
        ...
        <p class="text-teal fw-bold pb-3">賸餘款: {{ (project.ApprovedAmount - amount).toLocaleString() }}元</p>
        <div class="table-responsive mb-0">
            <table class="table align-middle gray-table">
                ...
                <tbody>
                    <tr>
                        <td class="text-end">{{ amount - prevAmount }}</td>
...
<script setup>
    ...
    const docs = ref([
        { Type: 21, Title: "經費收支單", Template: "../../Template/CUL/經費收支單.docx", Files: [] },
        { Type: 22, Title: "經費分攤表", Template: "../../Template/CUL/經費分攤表.docx", Files: [] },
        { Type: 23, Title: "補助清單", Template: "../../Template/CUL/補助清單.docx", Files: [], Excludes: ["1"] },
        { Type: 24, Title: "發票憑證", Files: [], Excludes: ["1","2"] }
    ]);
    ...
    const add = (item, file) => {
        if (file.Type === "application/pdf") {
            item.Files.push({
                Type: item.Type,
                Path: file.Path,
                Name: `${project.value.ProjectID}_第二期請款_${item.Title}.pdf`
            });
        }
    };
    ...
    const load = () => {
        api.culture("getPayment", { ID: props.id }).subscribe((result) => {
    ...
    const save = (submit) => {
        ...
        api.culture("savePayment", data).subscribe(() => {
            if (submit) {
                emit("next");
            } else {
                load();
