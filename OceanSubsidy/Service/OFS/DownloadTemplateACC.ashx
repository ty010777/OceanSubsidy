...
public class DownloadTemplateEDC : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            docName = "附件01-申請表.docx";
            string templatePath = context.Server.MapPath($"~/Template/ACC/{docName}");
            ...
            else
            {
                ...
                // 取得基本資料
                var data = accessibilityService.getApplication(new JObject { ["ID"] = id }, context);
                ...
                placeholder.Add("{{A9.3}}", jobj["Contacts"][1]["Phone"]?.ToString() ?? "");
        ...
        else if (templateType == "2")
        {
            ...
            else
            {
                var accessibilityService = new AccessibilityService();
                // 取得基本資料
                var data = accessibilityService.getWorkSchedule(new JObject { ["ID"] = id }, context);
                var jobj = JObject.Parse(JsonConvert.SerializeObject(data));
                ...
                string[] C5_Keys = { "##C5.1##", "##C5.2##", "##C5.3##", "##C5.4##", "##C5.5##" };
                var budgets = new List<Dictionary<string, string>>();
                foreach (var obj in jobj["BudgetPlans"] as JArray)
                {
                    if (obj["Deleted"]?.ToObject<bool>() == true) continue;
                    var dict = new Dictionary<string, string>();
                    dict[C5_Keys[0]] = obj["Title"]?.ToString() ?? "";
                    dict[C5_Keys[1]] = obj["Amount"]?.ToString() ?? "";
                    dict[C5_Keys[2]] = obj["OtherAmount"]?.ToString() ?? "";
                    int amt = int.TryParse(obj["Amount"]?.ToString(), out var a) ? a : 0;
                    int oamt = int.TryParse(obj["OtherAmount"]?.ToString(), out var b) ? b : 0;
                    dict[C5_Keys[3]] = (amt + oamt).ToString();
                    dict[C5_Keys[4]] = obj["Description"]?.ToString() ?? "";
                    budgets.Add(dict);
                }

                // Benefits
                data = accessibilityService.getBenefit(new JObject { ["ID"] = id }, context);
                jobj = JObject.Parse(JsonConvert.SerializeObject(data));
        ...
        {
            docName = "附件04-未違反公職人員利益衝突迴避法切結書及事前揭露表.docx";
            string templatePath = context.Server.MapPath($"~/Template/ACC/{docName}");
            if (!File.Exists(templatePath))
            {
                context.Response.StatusCode = 404;
            ...
            else
            {
                ...
                placeholder.Add("{{A7}}", jobj["Project"]["Address"]?.ToString() ?? "");
                // 取得負責人姓名
                data = accessibilityService.getApplication(new JObject { ["ID"] = id }, context);
