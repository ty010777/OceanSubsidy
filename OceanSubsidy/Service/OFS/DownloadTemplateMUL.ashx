...
public class DownloadTemplateEDC : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            docName = "1-申請表.docx";
            string templatePath = context.Server.MapPath($"~/Template/MUL/{docName}");
            ...
            else
            {
                ...
                // 取得基本資料
                var data = multipleService.getApplication(new JObject { ["ID"] = id }, context);
        ...
        else if (templateType == "2")
        {
            ...
            else
            {
                var multipleService = new MultipleService();
                // 取得基本資料
                var data = multipleService.getWorkSchedule(new JObject { ["ID"] = id }, context);
                var jobj = JObject.Parse(JsonConvert.SerializeObject(data));
                ...
                // Benefits
                data = multipleService.getBenefit(new JObject { ["ID"] = id }, context);
                jobj = JObject.Parse(JsonConvert.SerializeObject(data));
        ...
        else if (templateType == "4")
        {
            ...
            else
            {
                ...
                placeholder.Add("{{A7}}", jobj["Project"]["Address"]?.ToString() ?? "");
                // 取得負責人姓名
                data = multipleService.getApplication(new JObject { ["ID"] = id }, context);
