...
public class OFS_EdcOtherSubsidyHelper
{
    ...
    public static void insert(OFS_EdcOtherSubsidy model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_EDC_Other_Subsidy] ([PID],[Unit],[Amount],[Content],[CreateTime],[CreateUser])
    ...
    public static List<OFS_EdcOtherSubsidy> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_EdcOtherSubsidy model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_EDC_Other_Subsidy]
               SET [Unit] = @Unit
                  ,[Amount] = @Amount
                  ,[Content] = @Content
                  ,[UpdateTime] = GETDATE()
    ...
    private static OFS_EdcOtherSubsidy toModel(DataRow row)
    {
        return new OFS_EdcOtherSubsidy
        {
            ID = row.Field<int>("ID"),
