...
public class OFS_EdcBudgetPlanHelper
{
    ...
    public static void insert(OFS_EdcBudgetPlan model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_EDC_Budget_Plan] ([PID],[Title],[Amount],[OtherAmount],[Description],[CreateTime],[CreateUser])
    ...
    public static List<OFS_EdcBudgetPlan> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_EdcBudgetPlan model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_EDC_Budget_Plan]
               SET [Title] = @Title
                  ,[Amount] = @Amount
    ...
    private static OFS_EdcBudgetPlan toModel(DataRow row)
    {
        return new OFS_EdcBudgetPlan
        {
            ID = row.Field<int>("ID"),
