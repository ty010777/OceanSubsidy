...
public class OFS_LitContactHelper
{
    ...
    public static void insert(OFS_LitContact model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_LIT_Contact] ([PID],[Role],[Name],[JobTitle],[Phone],[PhoneExt],[MobilePhone],[EMail],[CreateTime],[CreateUser])
    ...
    public static List<OFS_LitContact> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_LitContact model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_LIT_Contact]
               SET [Role] = @Role
                  ,[Name] = @Name
                  ,[JobTitle] = @JobTitle
                  ,[Phone] = @Phone
    ...
    private static OFS_LitContact toModel(DataRow row)
    {
        return new OFS_LitContact
        {
            ID = row.Field<int>("ID"),
