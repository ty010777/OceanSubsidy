...
public class OFS_LitBenefitHelper
{
    ...
    public static void insert(OFS_LitBenefit model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_LIT_Benefit] ([PID],[Title],[Target],[Description],[CreateTime],[CreateUser])
    ...
    public static List<OFS_LitBenefit> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_LitBenefit model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_LIT_Benefit]
               SET [Title] = @Title
                  ,[Target] = @Target
    ...
    private static OFS_LitBenefit toModel(DataRow row)
    {
        return new OFS_LitBenefit
        {
            ID = row.Field<int>("ID"),
