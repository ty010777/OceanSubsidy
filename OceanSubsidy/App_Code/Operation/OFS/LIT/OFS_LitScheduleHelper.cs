...
public class OFS_LitScheduleHelper
{
    ...
    public static void insert(OFS_LitSchedule model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_LIT_Schedule] ([PID],[Type],[ItemID],[Deadline],[Content],[CreateTime],[CreateUser])
    ...
    public static List<OFS_LitSchedule> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_LitSchedule model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_LIT_Schedule]
               SET [Type] = @Type
                  ,[ItemID] = @ItemID
                  ,[Deadline] = @Deadline
                  ,[Content] = @Content
    ...
    private static OFS_LitSchedule toModel(DataRow row)
    {
        return new OFS_LitSchedule
        {
            ID = row.Field<int>("ID"),
