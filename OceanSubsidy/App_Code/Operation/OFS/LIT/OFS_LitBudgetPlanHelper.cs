...
public class OFS_LitBudgetPlanHelper
{
    ...
    public static void insert(OFS_LitBudgetPlan model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_LIT_Budget_Plan] ([PID],[Title],[Amount],[OtherAmount],[Description],[CreateTime],[CreateUser])
    ...
    public static List<OFS_LitBudgetPlan> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_LitBudgetPlan model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_LIT_Budget_Plan]
               SET [Title] = @Title
                  ,[Amount] = @Amount
    ...
    private static OFS_LitBudgetPlan toModel(DataRow row)
    {
        return new OFS_LitBudgetPlan
        {
            ID = row.Field<int>("ID"),
