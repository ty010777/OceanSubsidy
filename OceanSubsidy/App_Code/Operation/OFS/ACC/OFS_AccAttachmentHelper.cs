...
public class OFS_AccAttachmentHelper
{
    ...
    public static void insert(OFS_AccAttachment model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_ACC_Attachment] ([PID],[Stage],[Type],[Path],[Name],[CreateTime],[CreateUser])
    ...
    public static List<OFS_AccAttachment> query(int pid, int stage = 0)
    {
        DbHelper db = new DbHelper();
    ...
    private static OFS_AccAttachment toModel(DataRow row)
    {
        return new OFS_AccAttachment
        {
            ID = row.Field<int>("ID"),
