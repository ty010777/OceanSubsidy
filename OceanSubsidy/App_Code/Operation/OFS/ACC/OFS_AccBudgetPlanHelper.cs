...
public class OFS_AccBudgetPlanHelper
{
    ...
    public static void insert(OFS_AccBudgetPlan model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_ACC_Budget_Plan] ([PID],[Title],[Amount],[OtherAmount],[Description],[CreateTime],[CreateUser])
    ...
    public static List<OFS_AccBudgetPlan> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_AccBudgetPlan model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_ACC_Budget_Plan]
               SET [Title] = @Title
                  ,[Amount] = @Amount
    ...
    private static OFS_AccBudgetPlan toModel(DataRow row)
    {
        return new OFS_AccBudgetPlan
        {
            ID = row.Field<int>("ID"),
