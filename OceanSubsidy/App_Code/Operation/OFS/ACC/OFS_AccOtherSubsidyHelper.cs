...
public class OFS_AccOtherSubsidyHelper
{
    ...
    public static void insert(OFS_AccOtherSubsidy model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_ACC_Other_Subsidy] ([PID],[Unit],[Amount],[Content],[CreateTime],[CreateUser])
    ...
    public static List<OFS_AccOtherSubsidy> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_AccOtherSubsidy model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_ACC_Other_Subsidy]
               SET [Unit] = @Unit
                  ,[Amount] = @Amount
                  ,[Content] = @Content
                  ,[UpdateTime] = GETDATE()
    ...
    private static OFS_AccOtherSubsidy toModel(DataRow row)
    {
        return new OFS_AccOtherSubsidy
        {
            ID = row.Field<int>("ID"),
