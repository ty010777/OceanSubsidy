...
public class OFS_AccScheduleHelper
{
    ...
    public static void insert(OFS_AccSchedule model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_ACC_Schedule] ([PID],[Type],[ItemID],[Deadline],[Content],[CreateTime],[CreateUser])
    ...
    public static List<OFS_AccSchedule> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_AccSchedule model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_ACC_Schedule]
               SET [Type] = @Type
                  ,[ItemID] = @ItemID
                  ,[Deadline] = @Deadline
                  ,[Content] = @Content
    ...
    private static OFS_AccSchedule toModel(DataRow row)
    {
        return new OFS_AccSchedule
        {
            ID = row.Field<int>("ID"),
