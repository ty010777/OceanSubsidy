...
public class OFS_MulOtherSubsidyHelper
{
    ...
    public static void insert(OFS_MulOtherSubsidy model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_MUL_Other_Subsidy] ([PID],[Unit],[Amount],[Content],[CreateTime],[CreateUser])
    ...
    public static List<OFS_MulOtherSubsidy> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_MulOtherSubsidy model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_MUL_Other_Subsidy]
               SET [Unit] = @Unit
                  ,[Amount] = @Amount
                  ,[Content] = @Content
                  ,[UpdateTime] = GETDATE()
    ...
    private static OFS_MulOtherSubsidy toModel(DataRow row)
    {
        return new OFS_MulOtherSubsidy
        {
            ID = row.Field<int>("ID"),
