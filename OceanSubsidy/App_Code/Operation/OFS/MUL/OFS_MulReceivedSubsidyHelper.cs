...
public class OFS_MulReceivedSubsidyHelper
{
    ...
    public static void insert(OFS_MulReceivedSubsidy model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_MUL_Received_Subsidy] ([PID],[Name],[Unit],[Amount],[CreateTime],[CreateUser])
    ...
    public static List<OFS_MulReceivedSubsidy> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_MulReceivedSubsidy model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_MUL_Received_Subsidy]
               SET [Name] = @Name
                  ,[Unit] = @Unit
                  ,[Amount] = @Amount
                  ,[UpdateTime] = GETDATE()
    ...
    private static OFS_MulReceivedSubsidy toModel(DataRow row)
    {
        return new OFS_MulReceivedSubsidy
        {
            ID = row.Field<int>("ID"),
