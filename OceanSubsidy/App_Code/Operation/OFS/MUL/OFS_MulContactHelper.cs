...
public class OFS_MulContactHelper
{
    ...
    public static void insert(OFS_MulContact model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_MUL_Contact] ([PID],[Role],[Name],[JobTitle],[Phone],[PhoneExt],[MobilePhone],[EMail],[CreateTime],[CreateUser])
    ...
    public static List<OFS_MulContact> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_MulContact model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            UPDATE [OFS_MUL_Contact]
               SET [Role] = @Role
                  ,[Name] = @Name
                  ,[JobTitle] = @JobTitle
                  ,[Phone] = @Phone
    ...
    private static OFS_MulContact toModel(DataRow row)
    {
        return new OFS_MulContact
        {
            ID = row.Field<int>("ID"),
