...
public class OFS_MulItemHelper
{
    ...
    public static void insert(OFS_MulItem model)
    {
        DbHelper db = new DbHelper();

        db.CommandText = @"
            INSERT INTO [OFS_MUL_Item] ([PID],[Title],[Begin],[End],[Deadline],[Content],[CreateTime],[CreateUser])
    ...
    public static List<OFS_MulItem> query(int pid)
    {
        DbHelper db = new DbHelper();
    ...
    public static void update(OFS_MulItem model)
    {
        DbHelper db = new DbHelper();
    ...
    private static OFS_MulItem toModel(DataRow row)
    {
        return new OFS_MulItem
        {
            ID = row.Field<int>("ID"),
