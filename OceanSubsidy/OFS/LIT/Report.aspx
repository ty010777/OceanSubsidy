<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Report.aspx.cs" Inherits="OFS_LitReport" Culture="zh-TW" UICulture="zh-TW" MasterPageFile="~/OFS/CUL/Layout.master" %>
...
<asp:Content ContentPlaceHolderID="MainContent" runat="server">
    <div>
        ...
        <project-progress :excludes="[4]" :id="id" :step="5" type="literacy"></project-progress>
        ...
        <template :key="item" v-for="(item) in stages">
            <project-report :data="item" :id="id" @next="next" type="literacy" v-if="item.id === stage"></project-report>
        </template>
    </div>
</asp:Content>

<asp:Content ContentPlaceHolderID="FloatContent" runat="server">
    <project-report-review :id="id" :stage="stage" type="literacy" v-if="store.literacy.reportStatus === '審核中'"></project-report-review>
    <script>
        setupVueApp({
            setup() {
                ...
                onMounted(() => {
                    api.literacy("findApplication", { ID: id }).subscribe((result) => store.init("literacy", result.Project));
